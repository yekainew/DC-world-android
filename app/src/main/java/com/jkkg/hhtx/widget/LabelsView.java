...
public class LabelsView extends ViewGroup implements View.OnClickListener {
    ...
    private LabelsView.SelectType mSelectType;
    ...
    private LabelsView.OnLabelClickListener mLabelClickListener;
    private LabelsView.OnLabelSelectChangeListener mLabelSelectChangeListener;
    ...
    public enum SelectType {
        ...
        static LabelsView.SelectType get(int value) {
            switch (value) {
                case 1:
                    return NONE;
                case 2:
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            mSelectType = LabelsView.SelectType.get(type);
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        if (state instanceof Bundle) {
            ...
            //恢复标签的选择类型
            setSelectType(LabelsView.SelectType.get(bundle.getInt(KEY_SELECT_TYPE_STATE, mSelectType.value)));
            //恢复标签的最大选择数量
    ...
    public void setLabels(List<String> labels) {
        setLabels(labels, new LabelsView.LabelTextProvider<String>() {
            @Override
     ...
     * @param <T>
     */
    public <T> void setLabels(List<T> labels, LabelsView.LabelTextProvider<T> provider) {
        //清空原有的标签
        innerClearAllSelect();
        removeAllViews();
        mLabels.clear();
        ...
        if (mSelectType == LabelsView.SelectType.SINGLE_IRREVOCABLY) {
            setSelects(0);
        }
    }
    ...
    private <T> void addLabel(T data, int position, LabelsView.LabelTextProvider<T> provider) {
    ...
    public void onClick(View v) {
        if (v instanceof TextView) {
            ...
            if (mSelectType != LabelsView.SelectType.NONE) {
                if (label.isSelected()) {
                    if (mSelectType != LabelsView.SelectType.SINGLE_IRREVOCABLY
                            && !mCompulsorys.contains((Integer) label.getTag(KEY_POSITION))) {
                        setLabelSelect(label, false);
                    }
                } else if (mSelectType == LabelsView.SelectType.SINGLE || mSelectType == LabelsView.SelectType.SINGLE_IRREVOCABLY) {
                    innerClearAllSelect();
                    setLabelSelect(label, true);
                } else if (mSelectType == LabelsView.SelectType.MULTI
    ...
    public void clearAllSelect() {
        if (mSelectType != LabelsView.SelectType.SINGLE_IRREVOCABLY) {
            if (mSelectType == LabelsView.SelectType.MULTI && !mCompulsorys.isEmpty()) {
    ...
    public void setSelects(int... positions) {
        if (mSelectType != LabelsView.SelectType.NONE) {
            ...
            int count = getChildCount();
            int size = mSelectType == LabelsView.SelectType.SINGLE || mSelectType == LabelsView.SelectType.SINGLE_IRREVOCABLY
                    ? 1 : mMaxSelect;
            for (int p : positions) {
                if (p < count) {
    ...
    public void setCompulsorys(List<Integer> positions) {
        if (mSelectType == LabelsView.SelectType.MULTI && positions != null) {
            mCompulsorys.clear();
    ...
    public void setSelectType(LabelsView.SelectType selectType) {
        if (mSelectType != selectType) {
            ...
            if (mSelectType != LabelsView.SelectType.MULTI) {
                mCompulsorys.clear();
            }
        }
    }
     ...
     * @param l
     */
    public void setOnLabelSelectChangeListener(LabelsView.OnLabelSelectChangeListener l) {
